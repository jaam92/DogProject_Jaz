...
if (numArgs < 5):
	print "Missing arguments. Usage: python IBDSeqQualCounts.py IBDSegsRangeFile posFile posColumNumber outputFile"
	exit(1)
...
with open(outputFilename, 'w') as outputFile:
	for (lowerBound, upperBound) in ROHRanges:
		...
		outputFile.write(str(lowerBound) + tab + str(upperBound) + tab + str(COUNT_POS_IN_RANGE) + tab + str(LengthROH) + tab + str.format("{0:.6f}", propGoodSitesinROH) + endOfLine) 

...
df2 = df %>% 
  select(breed1 = breed2, breed2 = breed1,WeightedFst)
...
#plot
ggplot(data = mergeDF, aes(x=breed1, y=breed2,fill=WeightedFst), colour = "white") +  
  geom_tile() + 
  scale_fill_gradient(low = "blue", high = "red") + 
  theme_bw() + 
  theme(axis.text.x = element_text(size  = 24,angle=40, vjust=1, hjust=1), 

...
paste AutosomeCoords.bed mapChromsBackFromPlasmid.bed | awk '{print $1"\t""0""\t"$2"\t"$6"\t"$4"\t"$5}' | /u/home/j/jmooney3/klohmueldata/jazlyn_data/software/bedtools2/bin/bedtools intersect -a ForAbi_EnsemblGenes_CanFam3.1_SingleTranscript.bed -b stdin -wa -wb | awk '{print $1"\t"$2+$9"\t"$3+$9"\t"$4}' > ForAbi_EnsemblGenes_CanFam3.1_SingleTranscript_mapToPlasmid.bed
